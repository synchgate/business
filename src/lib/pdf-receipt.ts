  ...
  return `
    <div style="width:600px;background:${colors.surface};font-family:'Inter',Arial,sans-serif;padding:40px;box-sizing:border-box;">
      ...
      <div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:28px;">
        <div>
          <div style="font-size:20px;font-weight:700;color:${colors.primary};letter-spacing:-0.02em;">EBS</div>
          <div style="font-size:11px;color:${colors.muted};margin-top:2px;">ebs.entacrest.com</div>
        </div>
        <div style="text-align:right;">
          ...
          <div style="font-family:'Courier New',monospace;font-size:14px;font-weight:600;color:${colors.ink};">${invoice.invoice_number.split("-").slice(0, 3).join("-")}</div>
        </div>
      </div>
...
export async function generateReceiptPdf(
  ...
  try {
    ...
    pdf.save(`receipt-${invoice.invoice_number.split("-").slice(0, 3).join("-")}.pdf`);
  } finally {
